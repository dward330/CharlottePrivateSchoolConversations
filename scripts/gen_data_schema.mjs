...
for (const s of STRUCTURED) {
  ...
  if (s.topic === 'college-support') {
    ...
    w('**A RANKING label on this card must sit on the university that earns it FOR THAT')
    w('SCHOOL.** A rate here is a joint property of school \u00d7 university, so the campus that')
    w('is "the most selective of the six" differs per school and is not a fixed descriptor')
    w('of any campus. The card originally pinned both ranking labels into the shared')
    w('per-row `note` field -- `the most selective of the six` on UNC-Chapel Hill and')
    w('`largest admit rates of the six` on East Carolina. Across the 11 schools then')
    w('shipped, the first was false on 5 and the second on 6; only 2 were right on both.')
    w()
    w('The labels are worth keeping -- they tell a parent which campus is the hard one --')
    w('so they MOVE rather than being deleted. How the field is built:')
    w()
    w('- **Each `universities[].note` opens with an institutional half** (geography or')
    w('  role: `Flagship \u00b7 Chapel Hill`, `Hometown campus`, `Piedmont Triad`), and the')
    w('  earning campus gets the ranking half appended after a ` \u00b7 `.')
    w('- **Anchor both labels on the LATEST-TERM rate** -- the `Admit rate from this')
    w('  school` column a reader actually scans -- and quote the same term in the subhead')
    w('  and third stat tile, so the whole card speaks in one column. Ties break on the')
    w('  larger denominator: 25 of 25 outranks 8 of 8.')
    w('- **This anchor is a deliberate trade, settled 2026-08-20 on a review of Cannon.**')
    w('  The two columns can disagree: UNC Greensboro led Cannon\'s pooled five-year rate')
    w('  at 100% off 24 applicants, while UNC Charlotte showed 100% off 25 in Fall 2025,')
    w('  and the label on the thinner row read as simply wrong beside a column showing')
    w('  three campuses at 100.0%. The cost is that a label can now rest on very few')
    w('  applicants -- Charlotte Christian\'s is 3 of 3 -- which the denominator tiebreak')
    w('  limits but does not remove. Legibility beside the quoted column won.')
    w('- **`subhead` and the third stat tile name the same hard campus**, on the same')
    w('  anchor.')
    w('- **`npm run check:ncsuper` enforces all of it** and is chained into')
    w('  `npm run build`: each label must appear exactly once, on the campus holding the')
    w('  lowest (resp. highest) pooled rate.')
    w()
    w('Note this makes `note` a PER-SCHOOL string. Identical English still collapses to')
    w('one overlay entry by content hash, so the four purely institutional notes remain')
    w('shared while the two carrying a ranking half now differ per school -- which is')
    w('correct, and is what the translation layer must carry.')
    w()
    ...
    w()
    w('**Every `cats` bucket is derived from a master, never hand-judged — and two build')
    w('gates enforce it.** `cats` is hand-typed per college per school, and hand-typing')
    w('drifts: a college tagged `nu75` while the rank printed beside it read `#127` shipped')
    w('past a green build, and 10 colleges carried `p4` in some schools but not others.')
    w('When adding a school, tag from the masters rather than judging membership by eye —')
    w('the checkers will tell you either way, but the reasoning belongs in the master.')
    w()
    w('- `nu75` / `lac75` — National / Liberal rank **≤75**, inclusive, from the rank label')
    w('  the master resolves. A band (`National Rank #382-422`) never qualifies.')
    w('  `npm run check:buckets` (`scripts/check_selectivity_buckets.mjs`) gates both the')
    w('  tags AND the printed counts.')
    w('- `p4` / `hbcu` — membership in `src/data/collegeMemberships.ts` (`POWER_FOUR`, 68')
    w('  institutions incl. Notre Dame; `HBCUS`). Sourced in')
    w('  `source-material/college-support/_shared/`. `npm run check:memberships`')
    w('  (`scripts/check_memberships.mjs`).')
    w('- `ivy` / `ivyplus` — deliberately UNCHECKED. `ivy` is self-evident (the 8), and')
    w('  `ivyplus` is an editorial grouping with no settled definition to check against.')
    w()
    w('**A membership violation is a question, not a verdict.** The first run of')
    w('`check:memberships` reported 12 `hbcu` over-inclusions and all twelve were genuine')
    w('HBCUs missing from the roster — the tags were right and the table was wrong. Confirm')
    w('which side is actually wrong before editing either; "fixing" the data would have')
    w('deleted 12 correct tags and every check would then have reported clean.')
    w()
    w('**The bucket counts are INSTITUTIONS, not tagged rows, and they live on three')
    w('surfaces.** A school listing both Arizona State campuses has two rows and one')
    w('institution, so counts derive through `canonicalMember()` (`p4`/`hbcu`) or')
    w('`canonicalRanked()` (`nu75`/`lac75`). The same figure appears in')
    w('the `buckets` row, sometimes a stat tile, and `src/data/metricValues.ts` for the')
    w('Compare table; all three must move together. `check:memberships` gates all three —')
    w('the third was found only by opening the page, which rendered `65 / 68` and `57 / 68`')
    w('at once while every automated check passed.')
    w()
    w('**⚠️ UPDATING THE US NEWS EDITION MOVES THE TOP-75 COUNTS — they are derived, and')
    w('they are easy to forget.** A new edition rewrites `collegeRankings.ts`, which')
    w('silently changes how many institutions fall inside `nu75`/`lac75` for every school.')
    w('`check:buckets` only verified the *tags* until 2026-09-22, so the 2027 refresh')
    w('landed with all 11 schools still printing their 2026 counts (Cannon `46 / 75` when')
    w('66 qualified). It now also gates **four** printed surfaces — the `buckets` row, a')
    w('stat tile, the `metricValues.ts` Compare cell, and the qual sentence that spells')
    w('the number out in prose (`"58 of the top 75 …"`), a number that drifts exactly as')
    w('easily as one in a cell.')
    w()
    w('Two rules the counts follow. They are **clamped at the published denominator**:')
    w('ties put 79 National and 77 Liberal institutions inside the top-75 band, so a')
    w('saturated school reads `75 / 75` rather than `77 / 75`. And the prose that dates')
    w('the edition (`"matched against the 2027 U.S. News tables"`) must be re-dated in the')
    w('same pass — `Redesign Research 2026` is a dossier filename, not an edition, and')
    w('stays.')
    w()
