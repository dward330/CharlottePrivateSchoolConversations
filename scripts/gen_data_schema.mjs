...
for (const s of STRUCTURED) {
  ...
  if (s.topic === 'college-support') {
    ...
    w('  the master resolves. A band (`National Rank #395-434`) never qualifies.')
    w('  `npm run check:buckets` (`scripts/check_selectivity_buckets.mjs`).')
    ...
    w('institution, so counts derive through `canonicalMember()`. The same figure appears in')
