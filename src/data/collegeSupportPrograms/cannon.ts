...
export const cannon: CollegeSupportProgram = {
  ...
  outcomes: {
    ...
    caveat:
      'the headline list is explicitly “College Acceptances” across three graduating classes (2022, 2023, 2024) — roughly 330 students, each generating multiple acceptances. An institution appearing there does not mean a Cannon student attended, or that an acceptance happened in any particular year. Prefer the Class of 2023 matriculation list, which is real enrollment. Note also that the 2025–26 profile dropped the acceptance list entirely, so the newest available data is already one full cycle stale.',
    flags: [
      {
        kind: 'verify',
